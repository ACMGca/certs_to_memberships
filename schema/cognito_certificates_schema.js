...
export const getCognitoCertificateSchema = () => {
    ...
    const cognitoSchemaObject = {
        ...
        HikeTimeLimit: z.string().optional().nullable(),
        RockTimeLimit: z.string().optional().nullable(),
        AlpineTimeLimit: z.string().optional().nullable(),
        SkiTimeLimit: z.string().optional().nullable(),
    ...
    return z.object(cognitoCertificateSchema)
        .transform((data, ctx) => {
            ...
            // DATA_FIX_1
            // Keep only TimeLimits with a definitive year value otherwise, set to null
            const timeLimitFields = ['HikeTimeLimit', 'RockTimeLimit', 'AlpineTimeLimit', 'SkiTimeLimit']
            const matchesYear = /^\d{4}$/
            // check each timelimit field and keep only those with a year value
            timeLimitFields.forEach((timeLimitField) => {

                if(!matchesYear.test(data[timeLimitField])){
                    delete data[timeLimitField]
                }
            })
            return data
        })
